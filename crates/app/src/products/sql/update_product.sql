...
closed_current_detail AS (
    ...
    WHERE product_uuid = (SELECT uuid FROM target_product)
...
updated_product AS (
    UPDATE products
    SET updated_at = NOW()
    WHERE uuid = (SELECT product_uuid FROM inserted_detail)
    RETURNING uuid, created_at, updated_at, deleted_at
)
SELECT
...
INNER JOIN inserted_detail
    ON inserted_detail.product_uuid = updated_product.uuid
